...
//    AVDictionary *options[p_sys->ic->nb_streams ? p_sys->ic->nb_streams : 1];
    ...
    for( i = 0; i < p_sys->ic->nb_streams; i++ )
    {
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            if( cc->palctrl )
            {
                fmt.video.p_palette = (video_palette_t *)malloc( sizeof(video_palette_t) );			// sunqueen modify
            ...
            if( cc->codec_type == AVMEDIA_TYPE_ATTACHMENT )
            {
                ...
                if( cc->codec_id == AV_CODEC_ID_TTF )
                {
                    ...
                    if( filename && filename->value )
                    {
                        ...
                        TAB_APPEND( (input_attachment_t **), p_sys->i_attachments, p_sys->attachments,
                                p_attachment );			// sunqueen modify
                    }
                }
...
static int Demux( demux_t *p_demux )
{
    ...
    if( p_sys->ic->start_time != (int64_t)AV_NOPTS_VALUE )
    {
        q = cover_lldiv( p_sys->ic->start_time, AV_TIME_BASE);			// sunqueen modify
    ...
    {
        q = cover_lldiv( pkt.dts, p_stream->time_base.den );			// sunqueen modify
    ...
    {
        q = cover_lldiv( pkt.pts, p_stream->time_base.den );			// sunqueen modify
